...
import onlineSearchFormHtml from './onlineSearchFormContact.html';
// Apex method
...
const FIELDS = ["Contact.SecurityNumberParentNumber__c",
                "Contact.SecurityNumberBranchNumber__c", 
                "Contact.DestinationCode__c",
                "Contact.TypeCode__c"];
import { getRecord, getFieldValue } from "lightning/uiRecordApi";

export default class onlineSearchFormContact extends LightningElement {
    GCODE = '';
    GDATA = '';
    GYOUMU_ID = '';
    USER_ID = '';
    ...
    @wire(getRecord, 
        { recordId: '$recordId',
            ...
            if (error) {
                ...
                this.contact = data;
                this.SHOKEN_NO_OYA = this.contact.fields.SecurityNumberParentNumber__c.value;
                this.SHOKEN_NO_EDA = this.contact.fields.SecurityNumberBranchNumber__c.value;
                this.DAIRITEN_CD = this.contact.fields.DestinationCode__c.value;
                this.SHUMOKU = this.contact.fields.TypeCode__c.value;
            }
        }
    ...
    doConnectPattern1(evt){
        ...
        console.log(url);
        console.log(param);
        window.open (encodeURI(url + param), null , 'toolbar=yes,menubar=yes,scrollbars=yes');
        evt.stopPropagation();
    }
    ...
    doConnectPattern3(evt){
        ...
        let SHOKEN_NO_OYA_Regex = /^[0-9]{9,10}$/;
        if(!this.SHOKEN_NO_OYA.match(SHOKEN_NO_OYA_Regex)){
            isError = true;
            dispatchEvent(
                new ShowToastEvent({
